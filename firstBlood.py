...
def go():
	...
	for game in games:
		...
		# gameVersion = '.'.join(encjson['gameVersion'].split('.')[:2])

		# if patches:
		# 	if gameVersion in patches:
		# 		ignore = False
	...
	os.system('clear')
	htmlString += '-|-------------------|-------|-----------|<br>'
	htmlString += '-| Champion Name     | FB %  |Sample Size|<br>'
	htmlString += '-|-------------------|-------|-----------|<br>'
	...
	champDict = {}
	for eachteamname in q:
		champDict[champidtoname(eachteamname)] = eachteamname
		champDictSorted = sorted(champDict)

		champSort = []
		for champName in champDictSorted:
			champSort.append(champDict[champName])


	jsonArray = []
	for eachteamname in champSort:
		champName = champidtoname(eachteamname)
		sampleSize = str(y[eachteamname])
		fbPer = str(a[eachteamname])

		jsonEntry = {}
		jsonEntry['champName'] = champName
		jsonEntry['sampleSize'] = sampleSize
		jsonEntry['fbPer'] = fbPer


		jsonArray.append(jsonEntry)


	file = 'data/firstBlood.json'

	jsonEnc = json.dumps(jsonArray)
	f = open(file, 'w')
	f.write(jsonEnc)
	f.close()

	print('Json data stored in {}'.format(file))
	return htmlString